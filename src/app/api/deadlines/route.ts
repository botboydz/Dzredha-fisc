...
import { validateInput, deadlineCreateSchema, deadlineUpdateSchema } from "@/lib/validations";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const validation = validateInput(deadlineCreateSchema, body);
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    // Input validation with Zod
    const validation = validateInput(deadlineUpdateSchema, updateFields);
    if (!validation.success) {
      return NextResponse.json(
        { error: "Validation failed", details: validation.errors },
        { status: 400 }
